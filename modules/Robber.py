...
from modules.DialogBox.PasswordBox import PasswordBox
from modules.FileUtils import dump_table, load_table
from modules.htmlUtils import table_to_html
from modules.listUtils import (filter_with_keys, getSelected, mergeSelected,
                               sort_class)
...
class Robber(object):
    ...
    def _init_logger(self):
        ...
        handler = RotatingFileHandler(
            Config.file_name['log'], maxBytes=1024**2, backupCount=3
        )
    ...
    def login(self):
        ...
        user_name_box.set_prompt(String['userName']).set_pattern(
            '\d{10}').set_warning(String['user_name_formed'])
    ...
    def emailLogin(self):
        ...
        for item in list(Config.user.keys())[2:]:
            ...
            inputBox = InputBox()
            inputBox.set_prompt(
                String[item] + '(' + String['current'] + ': ' + current + ')')
        ...
        Mail.send_mail(String['just_test_connection'],
        ...
        while True:
            while True:
                ...
                if flag:
                    ...
                    break
                selectable_ = [
                    selectable for selectable in selectable_ if selectable[2] not in getSelected()]
                buttonId_ = [selectable[0] for selectable in selectable_ if int(
                    selectable[6]) > int(selectable[7])]

                if buttonId_:
                    random.shuffle(buttonId_)
                    buttonId = buttonId_[0]
                    self.output(Logger.log(
                        String['robbing_report'], level=Logger.warning))
                ...
                new_selected_ = [
                    selected for selected in selected_ if selected[2] not in getSelected()]
                new_selected_item_ = [selected[2]
                                      for selected in new_selected_]
                if new_selected_item_:
                    mergeSelected(new_selected_item_)
                    self.output(Logger.log(
                        String['robbed_new_reports'], subContent_=new_selected_item_, level=Logger.error))
                    selectedHtml = table_to_html(new_selected_)
                    if Mail.CONNECTED_TO_MAIL:
                        threading.Thread(target=Mail.send_mail, args=(
                            String['robbed_new_reports'], selectedHtml,)).start()
    ...
    def rob_class(self):
        while True:
            while True:
                ...
                if not selectable_:
                    print(Logger.log('没有课程可抢', subContent_=[
                          '正在退出...'], level=Logger.error))
                    return True
    ...
    def fetchGrade(self, callback, output):
        ...
        while True:
            while True:
                ...
                new_grade = [line for line in grade if line not in load_table(
                    Config.file_name['grade_cache'])]
                if new_grade:
                    output(new_grade)
                    ...
                    if Mail.CONNECTED_TO_MAIL:
                        ...
                        threading.Thread(target=Mail.send_mail, args=(
                            'Fetched new grade', grade_html,)).start()
